...
- **Direction C = restraint + one accent.** Tokens in `src/index.css` as `rgb(var(--x) / <alpha-value>)`: `ink / ink-muted / ink-subtle / ink-inverse`, `canvas / surface / surface-strong`, `border / border-strong`, `accent (+hover/text/soft/fg, #6D3BF5 light / #7C4DFF dark)`, `success / warning / danger (+soft)`. **Pages must never use a raw palette class (`slate-*`, `violet-*`) or an `isDark ? … : …` colour ternary again** — `isDark` is only for theme-toggle icons.
- Type: **Inter Variable** (`@fontsource-variable/inter`), scale `display/h1/h2/h3/h4/lead/body/small/overline` in tailwind.config. **Sentence case** — uppercase only on the 11px `.overline` label. `font-display` still resolves (to Inter) so old code compiles.
- 4 radii (6/8/12/16) + pill; `2xl`/`3xl` collapse into the scale. Elevation `shadow-e1…e4` (layered micro-shadows, not glows). Layout: `.wrap` 1120px / `.wrap-narrow` 736px / `.section`.
- **Primitives (use these, don't hand-roll):** `ui/button` (primary|ink|secondary|subtle|ghost|danger|link × sm|md|lg|icon, `pill`/`block`), `ui/panel` (Panel/PanelHeader/PanelTitle/PanelDescription), `ui/section` (Section + SectionHeading eyebrow/title/subtitle), `ui/badge`, `ui/stat`, `ui/empty-state`, `ui/page-header`, `ui/field` (Field/TextInput/Select), `ui/data-table` (TableWrap/Table/Th/Td/Tr), `ui/stepper`, `brand/logo`, `landing/product-frame`.
...
- Remaining gaps: no real product screenshots (ProductFrame still wraps the SVG illustrations); no social proof/logos/compliance page; dashboard data still mock and now contradicts `/profile`.
